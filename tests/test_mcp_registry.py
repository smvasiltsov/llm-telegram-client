...
class SkillRegistryTests(unittest.TestCase):
    def test_discover_loads_valid_skill(self) -> None:
        ...
        self.assertEqual(record.spec.skill_id, "echo")
    ...
    def _write_good_skill(skills_pkg: Path) -> None:
        ...
        (good_dir / "skill.py").write_text(
            textwrap.dedent(
                """
                from app.mcp.skills_contract import SkillSpec, SkillResult

                class GoodSkill:
                    def describe(self):
                        return SkillSpec(skill_id=\"good\", name=\"Good\", version=\"0.0.1\")

                    def validate_config(self, config):
                        return []

                    def run(self, ctx, payload):
                        return SkillResult(status=\"ok\", output={\"ok\": True})
