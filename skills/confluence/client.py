...
class ConfluenceHTTPError(ConfluenceClientError):
...
class ConfluenceClient:
    def __init__(self, *, config: ConfluenceClientConfig, username: str, token: str) -> None:
            ...
            try:
                ...
                if exc.code == 409:
                    raise ConfluenceHTTPError(
                        status_code=409,
                        message="Confluence version conflict (409)",
                        response_body=body,
                    ) from exc
                ...
                raise ConfluenceHTTPError(
                    ...
                    message=f"Confluence HTTP error {exc.code}",
                    response_body=body,
                ) from exc
                ...
                if attempt < self._retry_attempts:
                    time.sleep(self._retry_sleep_sec)
                    continue
                raise ConfluenceClientError(f"Confluence request failed after {attempt} attempts: {exc}") from exc

        raise ConfluenceClientError("Confluence request failed")
    ...
    def delete_json(
        ...
        *,
        ...
        return self.request_json(method="DELETE", path=path, params=params)
...
def _parse_json_object(raw: bytes) -> dict[str, Any]:
    try:
        ...
        raise ConfluenceClientError(f"Invalid JSON response: {exc}") from exc
